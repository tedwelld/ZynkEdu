...
public sealed class PlatformSubjectCatalogServiceTests
{
    ...
    public async Task UpdateAndDeleteAsync_ManageCatalogSubjects()
    {
        ...
        var service = CreateService(context);
        var created = await service.CreateAsync(new CreateSubjectRequest("Biology", "BIO1", "ZGC Level"));

        var updated = await service.UpdateAsync(created.Id, new UpdateSubjectRequest("Advanced Biology", null, "A'Level"));
    ...
    public async Task ImportFromSchoolToCatalogAsync_SkipsDuplicatesAndKeepsTheGradeLevel()
    {
        ...
        context.Subjects.AddRange(
            new Subject { SchoolId = 20, Code = "MATH1", Name = "Mathematics", GradeLevel = "ZGC Level" },
            new Subject { SchoolId = 20, Code = "SCI1", Name = "Science", GradeLevel = "O'Level" });
    ...
    public async Task PublishAllCatalogToSchoolAsync_SkipsExistingMatches()
    {
        ...
        context.PlatformSubjectCatalogs.AddRange(
            new PlatformSubjectCatalog { Code = "MATH1", Name = "Mathematics", GradeLevel = "ZGC Level" },
            new PlatformSubjectCatalog { Code = "ENG1", Name = "English", GradeLevel = "O'Level" });
        context.Subjects.Add(new Subject
        {
            SchoolId = 41,
            Code = "MATHX",
